...
__global__ void ProdKernel(const T* input, T* output, int num_elements,
                           TensorInfo in_info, TensorInfo eff_out_info) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename T>
void LaunchProd(const T* input, T* output, int input_size, const int* in_dims,
                const int* in_strides, const int* eff_out_strides, int rank,
                musaStream_t stream) {
  TensorInfo in_info;
  ...
  ProdKernel<T><<<grid_size, block_size, 0, stream>>>(input, output, input_size,
                                                      in_info, eff_out_info);
}

template void LaunchFillOne<float>(float*, int, musaStream_t);
template void LaunchFillOne<int>(int*, int, musaStream_t);

template void LaunchProd<float>(const float*, float*, int, const int*,
                                const int*, const int*, int, musaStream_t);
template void LaunchProd<int>(const int*, int*, int, const int*, const int*,
                              const int*, int, musaStream_t);
