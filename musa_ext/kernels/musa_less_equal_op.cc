...
class MusaLessEqualOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    //         如果 float16/int64 在广播时也崩，那 muDNN
    //         就暂不支持这些类型的广播。
    ...
    if (input_0.shape() == input_1.shape()) {
      ...
      OP_REQUIRES_OK(
          ctx, ctx->allocate_temp(DT_BOOL, flat_shape_in0, &temp_output, attr));
      ...
      OP_REQUIRES(ctx, status == ::musa::dnn::Status::SUCCESS,
                  errors::Internal("MUSA LessEqual execution failed. Status: ",
                                   (int)status));
      ...
      auto copy_status =
          musaMemcpy(dst_ptr, src_ptr, output->NumElements() * sizeof(bool),
      ...
      Tensor input_0_view;
      Tensor input_1_view;
      Tensor output_view;  // 这里可以用 View，因为 4D 形状不会被 Python 误解为
                           // 1D (只要秩对即可)

      CHECK(
          input_0_view.CopyFrom(input_0, get_padded_4d_shape(input_0.shape())));
      CHECK(
          input_1_view.CopyFrom(input_1, get_padded_4d_shape(input_1.shape())));
      ...
      OP_REQUIRES(ctx, status == ::musa::dnn::Status::SUCCESS,
                  errors::Internal(
                      "MUSA LessEqual Broadcast execution failed. Status: ",
                      (int)status));
    }
  }
};
...
#define REGISTER_MUSA_LESS_EQUAL(TYPE)                            \
  REGISTER_KERNEL_BUILDER(                                        \
      Name("LessEqual").Device("MUSA").TypeConstraint<TYPE>("T"), \
